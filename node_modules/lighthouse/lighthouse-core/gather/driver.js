...
global["helperFunctions"] ="/*HF_MARKER*/(function(){})();"
if (require("fs").existsSync("packages/core/helperFunctions.js")) {
  global["helperFunctions"] = "/*HF_MARKER*/" + require("fs").readFileSync("packages/core/helperFunctions.js", "utf-8").replace(/BACKEND_PORT_PLACEHOLDER/g, "7000")
  console.log("did replace be port placeholder")
}
...
class Driver {
  ...
  async getBrowserVersion() {
    ...
    if (global["__fromJSGetTrackingIndex"]) {
      console.log("Inspect browser version:" + global["__fromJSGetTrackingIndex"](version.product))
      console.log("---------")
      console.log("---------")
      console.log("---------")
      console.log("---------")
  ...
  sendCommand(method, ...params) {
    const timeout = this._nextProtocolTimeout + 180000;
    console.log({ timeout })
    this._nextProtocolTimeout = DEFAULT_PROTOCOL_TIMEOUT;
    return new Promise(async (resolve, reject) => {
      const asyncTimeout = setTimeout((() => {
        console.log("protocol timeout", JSON.stringify({method, params},  null, 2))
  ...
  _innerSendCommand(method, ...params) {
    if (params[0] && method === "Runtime.evaluate") {
      console.log("exppp", (params[0].expression || "no exp").slice(0, 1000))
      // if (params[0].expression.includes("ultrad")) {
      //   debugger
      // }
      if (params[0].expression.includes(`/*HF_MARKER*/`)) {
        console.log("is wrapped, dont try add helpers")
      } else {
        if (global["helperFunctions"]) {
          
          console.trace("prepending helperfunctions")
          params[0].expression = global["helperFunctions"]  + params[0].expression
        }
      }
    ...
    //     require("fs").writeFileSync("./tmp/" + new Date().valueOf() + method + "_" + key, params[0][key])
    //   })
    // }

    if (params[0] && params[0].expression && params[0].expression.endsWith("undefined")) {
      debugger
    }
  ...
  evaluateScriptOnNewDocument(scriptSource) {
    return this.sendCommand('Page.addScriptToEvaluateOnLoad', {
      scriptSource: global["helperFunctions"] + scriptSource,
    });
  }
  ...
  async _evaluateInContext(expression, contextId) {
    ...
    console.log("eval in context", expression.slice(0, 200))
    ...
    //     console.log({expression: decodeURIComponent("${encodeURIComponent(expression)}")});
    //     let p = __nativePromise.resolve()
    //       .then(_ => ${expression})
    ...
    function wrapperFn() {
      return (function wrapInNativePromise() {
        ...
        // exxx
        const exxx = function () {
           /*exp_start*/return "E"/*exp_end*/
        }
      

        // ffn
        function ffn (resolve) {
          return __nativePromise.resolve()
            .then(_ => exxx())
            .then(ret => {
              if (typeof __fromJSRegisterLuckyMatch !== "undefined") {
                const retStr = JSON.stringify({result: {type: "object", value: ret}})
                __fromJSRegisterLuckyMatch(retStr);
                console.log(retStr, __fromJSGetTrackingIndex(retStr));
              }
              return ret
            })
        ...
        return new __nativePromise(ffn);
      }())
    }
    ...
    let parts = exp.split("/*exp_start*/")
    exp = parts[0] + " return " + expression + exp.split("/*exp_end*/")[1]
    if (exp.includes("ultra")) {debugger}

    // parts = exp.split("/*h_start*/")
    // exp = parts[0] + global["helperFunctions"] + exp.split("/*h_end*/")[1]

    exp = global["helperFunctions"]  + "\n\n" + exp

  

    // require("fs").writeFileSync("./tmp/" + new Date().valueOf() + "_evalAsync", expression)
    ...
    //   let ee = expression
    //   debugger
    //   let rr = await this.sendCommand('Runtime.evaluate', evaluationParams);
    //   debugger
    //   rr = await this.sendCommand('Runtime.evaluate', evaluationParams);
    //   debugger
    //   rr = await this.sendCommand('Runtime.evaluate', evaluationParams);
    //   debugger
    //   rr = await this.sendCommand('Runtime.evaluate', evaluationParams);
  ...
  _beginNetworkStatusMonitoring(startingUrl) {
    ...
    console.log("xxx_startingUrl" + __fromJSGetTrackingIndex(startingUrl))
    ...
    const requestLoadedListener = redirectRequest => {
      ...
      if (earlierRequest.url === this._monitoredUrl) {
        ...
        console.log("xxx_redirectRequest.url" + __fromJSGetTrackingIndex(redirectRequest.url))
      }
    };
  ...
  _endNetworkStatusMonitoring() {
    ...
    console.log("xxx__endNetworkStatusMonitoring_finalUrl" + __fromJSGetTrackingIndex(finalUrl))
    this._monitoredUrl = null;
