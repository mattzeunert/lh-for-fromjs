...
class Connection {
  ...
  handleRawMessage(message) {
    if (process.env.SAVE_MESSAGES) {
      console.log("handleRawMessage", (message + "").slice(0,200))
      const filePath = "./tmp/"+ new Date().valueOf() + "_handle_" + message.slice(0, 35).replace(/[^a-zA-Z0-9]/g, "-")+".txt"
      console.log("writing ", filePath)
      require("fs").writeFileSync(filePath, JSON.stringify(JSON.parse(message), null, 2) );
    }
    ...
    if (callback) {
      ...
      callback.resolve(Promise.resolve().then(_ => {
        ...
        log.formatProtocol('method <= browser OK',
          ...
          if (global["__fromJSCallFunctionWithTrackingChainInterruption"]) {
            // Try to match protocol response to in-browser value via lucky matches
            let resJSON = global["__fromJSCallFunctionWithTrackingChainInterruption"](function(){
              return JSON.stringify(res)
            })
            console.log("resjson", resJSON.slice(0,100) )
